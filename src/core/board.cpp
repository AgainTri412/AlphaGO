...
//Inspect current side
Player Board::getSideToMove() const {
    return side_to_move;
}

//Force a specific side to move
void Board::setSideToMove(Player p) {
    if (side_to_move != p) {
        // Toggling side_to_move must toggle the zobristSide flag exactly once.
        hashKey ^= zobristSide;
        side_to_move = p;
    }
}

//Place a stone for a specific Player without toggling turn;
bool Board::placeStone(int x, int y, Player player) {
    ...
    bb[p][c] |= (1ULL << off);
    hashKey ^= zobristTable[x][y][p];   // update piece hash only

    // NOTE: we do NOT touch side_to_move or zobristSide here.
    return true;
}
//Place a stone for a specific Player without toggling turn;
bool Board::removeStone(int x, int y, Player player) {
    if (x < 0 || x >= 12 || y < 0 || y >= 12) return false;
    ...
    if ((bb[p][c] & mask) == 0ULL) {
        // There is no stone of this player here; treat as a no-op or return false.
        return false;
    }

    bb[p][c] &= ~mask;
    hashKey ^= zobristTable[x][y][p];   // remove piece from hash

    // NOTE: no side_to_move / zobristSide change.
    return true;
}
